...
import { api, ApiError, completeEvent, type Account, type Candidate, type DemoAccount, type DemoAccounts, type Employee, type Metrics, type Profile, type Recommendations } from './api';
...
function App() {
    ...
    async function refreshDemo() { const result = await api<DemoAccounts>('/auth/demo-accounts'); setDemo(result.enabled ? result.accounts : []); }
    async function refreshAfterImport() {
        const [employees, overview, accounts] = await Promise.all([
            api<Employee[]>('/employees'), api<Metrics>('/hr/overview'), api<DemoAccounts>('/auth/demo-accounts'),
        ]);
        setPeople(employees); setMetrics(overview); setDemo(accounts.enabled ? accounts.accounts : []);
        setCelebration(null);
        setRecommendation(null);
        const currentId = profile?.employee.id;
        // Replacing demo can remove the previously opened employee entirely.
        setProfile(null);
    ...
    async function complete(event: Candidate) {
        ...
        await run('complete', async () => {
            const result = await completeEvent(oldProfile.employee.id, event);
