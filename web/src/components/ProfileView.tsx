...
import { factorLabels, formatDate, milestones, percent, recommendationLabel, statusLabels, typeLabels } from '../view-model';
...
function QuestCard({ event, index, profile, disabled, employee, onComplete }: {
    ...
    <h3>{event.title}</h3>
    {event.session_date && <p className="session-date"><Icon name="clock"/>Сессия: <time dateTime={event.session_date}>{formatDate(event.session_date)}</time></p>}
    {event.critical_benefit > 0 && <p className="critical-note">Критические навыки: закрывает {event.critical_benefit} ур. разрыва</p>}
...
export function ProfileView({ profile, recommendation, section, employee, busy, onRecommend, onComplete, onNavigate, celebration, clearCelebration }: {
    ...
    const title = section === 'quests' ? 'Каждый квест — шаг вперёд' : section === 'skills' ? 'Ваши навыки, ваша опора' : section === 'history' ? 'Путь складывается из шагов' : section === 'achievements' ? 'Маленькие победы. Большой путь.' : highestGrade ? 'Достигнут максимальный грейд' : `${employee ? 'Ваш следующий уровень' : 'Траектория сотрудника'}`;
    ...
    {section === 'profile' && <>
      <section className="journey-hero" aria-label="Путь к следующему грейду">
        <div className="hero-content"><span className="hero-label"><span />ВАША КАРЬЕРНАЯ ЭКСПЕДИЦИЯ</span><h2>{highestGrade ? <>Достигнут <em>максимальный грейд</em></> : trajectory.next_grade ? <>Курс на <em>{trajectory.next_grade}</em></> : <>Ваш путь <em>продолжается</em></>}</h2><p>{highestGrade ? 'Для вашего грейда в каталоге нет следующего уровня. Ваши навыки и история развития остаются доступны.' : trajectory.next_grade ? 'Не просто пройти обучение. Получить навыки, которые открывают следующий уровень.' : 'Требования следующего грейда пока не заданы. HR поможет уточнить вашу траекторию.'}</p><button className="lime-button" onClick={() => onNavigate(highestGrade ? 'skills' : 'quests')}>{highestGrade ? 'Посмотреть навыки' : 'Выбрать следующий квест'} <Icon name="arrow"/></button><div className="hero-caption"><Icon name="shield"/> Свой темп. Добровольные шаги. Реальный прогресс.</div></div>
        <div className="journey-map" aria-hidden="true"><div className="map-grid"/><svg viewBox="0 0 430 250" className="map-route"><path d="M48 202C110 235 112 83 187 118S251 239 302 135 311 40 374 46" fill="none" stroke="#426755" strokeWidth="2" strokeDasharray="5 7"/><path d="M48 202C110 235 112 83 187 118" fill="none" stroke="#c4eb8c" strokeWidth="3"/></svg><div className="map-node start"><Icon name="check"/><span>{person.grade}<small>Ваш текущий грейд</small></span></div><div className="map-node current"><Icon name="compass"/><span>{highestGrade ? 'Максимальный грейд' : 'Следующий квест'}<small>Вы здесь</small></span></div><div className="map-node middle"><Icon name="spark"/></div><div className="map-node finish"><Icon name="flag"/><span>{highestGrade ? person.grade : trajectory.next_grade ?? 'Новая цель'}<small>Ваш ориентир</small></span></div><span className="map-note">МАЛЕНЬКИЕ ШАГИ → БОЛЬШИЕ ВОЗМОЖНОСТИ</span></div>
      </section>
    ...
    <div className={section === 'profile' ? 'growth-grid' : ''}>
      {show('quests') && highestGrade && <section className="panel"><Icon name="flag"/><h2>Достигнут максимальный грейд</h2><p className="muted">Следующий грейд в текущем каталоге не предусмотрен. Можно посмотреть свои навыки и историю развития.</p></section>}
      {show('quests') && !highestGrade && <section className="quests-section" id="quests"><div className="section-heading"><div><span className="eyebrow">ВАШ ПЕРСОНАЛЬНЫЙ МАРШРУТ</span><h2>Следующие квесты <span className="count">{recommendation?.items.length ?? '—'}</span></h2></div><button className="secondary compact" disabled={!!busy} onClick={onRecommend}><Icon name={recommending ? 'clock' : 'spark'}/>{recommending ? 'Подбираем…' : recommendation ? 'Обновить подбор' : 'Подобрать квесты'}</button></div>
        ...
        {recommendation && <><div className={`mode-banner ${recommendation.mode === 'ai' ? 'ai' : ''}`}><Icon name={recommendation.cached ? 'clock' : recommendation.mode === 'ai' ? 'spark' : 'compass'}/><div><strong>{recommendationLabel(recommendation)}</strong><small>{recommendation.cached ? 'Исходный расчёт' : 'Время подбора'}: {recommendation.elapsed_ms} мс</small>{recommendation.mode === 'ai' && <small>{recommendation.provider} · {recommendation.model}</small>}</div></div>{recommendation.reason && <p className="reason-note">{recommendation.reason}</p>}{!recommendation.items.length && <div className="empty-quest small-empty"><Icon name="leaf"/><h3>Сейчас подходящих квестов нет</h3><p>Это не оценка вашего прогресса. Возможно, нужно уточнить навыки или дополнить каталог с HR.</p></div>}<div className="quest-list" aria-busy={recommending}>{recommendation.items.map((event, index) => <QuestCard key={`${event.id}:${event.occurrence_id ?? 'once'}`} event={event} index={index} profile={profile} disabled={!!busy} employee={employee} onComplete={setConfirm}/>)}</div></>}
        <details className="catalog"><summary>Все доступные активности <span>{profile.available.length}</span></summary>{profile.available.length ? profile.available.map(c => <div key={`${c.id}:${c.occurrence_id ?? 'once'}`}><Icon name="book"/><span><strong>{c.title}</strong><small>{typeLabels[c.type] ?? c.type}{c.session_date && <> · Сессия: {formatDate(c.session_date)}</>} · {Object.values(c.changes).map(s => `${s.name}: ${s.before} → ${s.after}`).join(' · ')}</small></span></div>) : <p className="muted">Каталог не содержит доступных шагов для этого профиля.</p>}</details>
      </section>}
      {show('skills') && <aside className="skills-column"><section className="panel skill-panel"><div className="section-heading"><h2>Навыки для роста</h2><Icon name="chart"/></div><p className="muted small">{person.grade} <span aria-hidden="true">→</span> {highestGrade ? 'Максимальный грейд' : trajectory.next_grade ?? 'Цель уточняется'}</p>{!highestGrade && <><div className="coverage-row"><strong>{coverage === null ? '—' : `${coverage}%`}</strong><span>покрытие требований<br />следующего грейда</span></div><Meter value={coverage} label="Покрытие требований следующего грейда"/><p className="fine-print">Покрытие навыков не означает автоматическое повышение.</p></>}{trajectory.critical_requirements_met !== null && <p className={`critical-summary ${trajectory.critical_requirements_met ? 'met' : 'pending'}`}><Icon name="flag"/>{trajectory.critical_requirements_met ? 'Требования по критическим навыкам выполнены' : 'Есть разрывы по критическим навыкам следующего грейда'}</p>}<div className="skill-list">{trajectory.skills.map(s => <div className={`skill ${s.critical ? 'critical-skill' : ''}`} key={s.id}><div className="skill-heading"><strong>{s.name}{s.critical && <span className="critical-tag">Критический</span>}</strong><span>{s.level ?? '?'} <small>/ {s.required}</small></span></div><Meter value={s.level === null ? null : percent(s.level / Math.max(1, s.required) * 100)} label={`${s.name}: ${s.level ?? 'неизвестно'} из ${s.required}`}/><small className={s.gap === 0 ? 'skill-done' : ''}>{s.gap === null ? 'Нужно уточнить уровень' : s.gap === 0 ? '✓ На уровне цели' : `До цели ещё ${s.gap} ур.`}</small></div>)}</div>{!trajectory.skills.length && <p className="empty-text">{highestGrade ? 'Достигнут максимальный грейд. Требований следующего уровня нет.' : 'HR ещё не добавил требования следующего уровня.'}</p>}<details className="all-skills"><summary>Все текущие навыки</summary><ul>{Object.entries(person.skills).map(([id, level]) => <li key={id}><span>{trajectory.skills.find(s => s.id === id)?.name ?? id.replace(/^SK_/, '').replaceAll('_', ' ')}</span><b>{level} / 5</b></li>)}</ul></details></section>
    ...
    {show('history') && <section className="panel history-section"><div className="section-heading"><div><span className="eyebrow">КАЖДЫЙ ШАГ ИМЕЕТ ЗНАЧЕНИЕ</span><h2>История вашего пути</h2></div><label className="filter-label">Показать<select value={historyFilter} onChange={e => setHistoryFilter(e.target.value)}><option value="all">Все активности</option>{Object.entries(statusLabels).map(([status, label]) => <option key={status} value={status}>{label}</option>)}{Array.from(new Set(profile.history.map(item => item.status))).filter(status => !(status in statusLabels)).map(status => <option key={status} value={status}>{status}</option>)}</select></label></div>{filteredHistory.length ? <ul className="timeline">{filteredHistory.map(h => <li key={h.id}><span className={`timeline-icon ${h.status === 'completed' ? 'done' : ''}`}><Icon name={h.status === 'completed' ? 'check' : 'clock'}/></span><div><h3>{h.title}</h3><time dateTime={h.occurred_at}>{formatDate(h.occurred_at)}</time></div><span className={`status ${h.status === 'completed' ? 'completed' : ''}`}>{statusLabels[h.status] ?? h.status}</span></li>)}</ul> : <p className="empty-text">{profile.history.length ? 'Нет активностей с выбранным статусом.' : 'Здесь появятся ваши активности. Начните с первого квеста.'}</p>}</section>}
    <dialog ref={dialog} className="complete-dialog" onCancel={() => setConfirm(null)} onClose={() => setConfirm(null)} aria-labelledby="complete-title"><span className="large-icon"><Icon name="flag"/></span><h2 id="complete-title">Квест действительно пройден?</h2><p>{confirm?.title}</p>{confirm?.session_date && <p>Сессия: {formatDate(confirm.session_date)}</p>}<p className="muted">Отмечайте только завершённую активность. Сервер обновит навыки и сохранит результат в истории.</p><div className="dialog-actions"><button className="secondary" autoFocus onClick={() => setConfirm(null)}>Ещё прохожу</button><button className="primary" disabled={!!busy} onClick={() => { if (confirm)
