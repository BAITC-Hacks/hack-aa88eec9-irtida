...
function profile(id) {
  ...
  const event = { id: 'design-lab', title: 'Архитектура сервиса: от идеи до решения', type: 'workshop', changes: { design: { name: 'System Design', before: level, after: Math.min(4, level + 1), gain: 1 } }, evidence: [
  ...
  return { employee, trajectory: { next_grade: id === 'empty' ? null : 'Senior', coverage: unknown || id === 'empty' ? null : Math.round((level + 4) / 11 * 100), status: 'in_progress', skills: id === 'empty' ? [] : [
    { id: 'design', name: 'System Design', level: unknown ? null : level, required: 4, gap: unknown ? null : 4 - level },
    { id: 'python', name: 'Python', level: 3, required: 4, gap: 1 },
    { id: 'speaking', name: 'Public Speaking', level: 1, required: 3, gap: 2 },
  ] }, history: [...(s.done ? [{ id: 'done', title: event.title, status: 'completed', occurred_at: '2026-09-23' }] : []), { id: 'miss', title: 'Клуб публичных выступлений', status: 'missed', occurred_at: '2026-08-01' }], available: s.done || ['empty', 'unknown'].includes(id) ? [] : [event], recommendation: s.recommendation ? { ...s.recommendation, cached: true } : null };
}
...
http.createServer(async (req, res) => {
  try {
    ...
    if (path === '/api/v1/hr/overview') return send(res, 200, { employee_count: 3, gaps: [{ name: 'System Design', affected: 1, eligible: 1, unknown: 1, percent: 100 }, { name: 'Навык без оценки', affected: 0, eligible: 0, unknown: 3, percent: null }], no_step: [{ id: 'empty', name: 'Демо: нет требований', reason: 'no_grade_rule' }], participation: [{ id: 'lab', title: 'Архитектура сервиса: от идеи до решения', completed: state('rules').done ? 1 : 0, missed: 1, declined: 1 }] });
    if (path === '/api/v1/imports') { if (id !== 'hr') return send(res, 403, { detail: 'HR only' }); const payload = JSON.parse(body); if (!payload.employees) return send(res, 422, { detail: [{ loc: ['body', 'employees'], msg: 'Отсутствует список профилей' }] }); return send(res, 200, { employees_added: payload.employees.length, history_added: (payload.history ?? []).length }); }
